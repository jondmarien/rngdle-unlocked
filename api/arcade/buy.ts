...
import { buyArcadeUpgrade } from '../../server/arcade.js';
import { createDb } from '../../server/db/index.js';
import { createLogger } from '../../server/logger.js';
import { LIMITS } from '../../server/rateLimit.js';
import { defineHandler } from '../../server/vercel-adapter.js';

const log = createLogger('api/arcade/buy');

/** POST /api/arcade/buy — { upgradeId } */
export default defineHandler(async (request) => {
  ...
  try {
    const result = await buyArcadeUpgrade(db, userId, upgradeId);
    if (!result.ok) return result.response;
    return Response.json({ run: result.run });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    log.error('buy failed', { userId, err: message });
    return Response.json({ error: message || 'Buy failed' }, { status: 500 });
  }
});
