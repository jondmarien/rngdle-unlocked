> Canonical in-repo copy: `docs/ARCHITECTURE.md`. Diagrams and trust model.
...
| ------------------ | ------------------ | ------------------------------------------------- | ------------------------------------------------------------------------------ |
| **Free play**      | Browser CSPRNG     | localStorage â†’ sync as `source=client`            | **Leaderboard â†’ Practice** + **All-Time** (via sync); no Ranked crowns         |
| **Ranked**         | Server CSPRNG      | Neon `source=ranked` first; client merges history | **Leaderboard â†’ Ranked**, community crowns, overtakes; also bumps **All-Time** |
| **Daily / Weekly** | Deterministic seed | sync as `source=challenge`                        | Practice + All-Time (public); not Ranked crowns                                |
| **Arcade**         | Server CSPRNG      | `arcade_*` tables only (Digits)                   | **Leaderboard â†’ Arcade** (best Digits run); Digits â‰  EP                        |
...
Server is source of truth for Digits, shop, bust, and cash-out. Client UI (`ArcadeScreen` + `arcade-api.ts`) only displays and requests mutations â€” it cannot fabricate run scores.
...
**Schema:** `arcade_meta` (unlocked upgrades, best Digits, lifetime cashed) Â· `arcade_runs` (one active run per user) Â· `arcade_run_rolls` (audit trail). Migration: `scripts/migrate-arcade.mjs`.
...
sequenceDiagram
  ...
  H->>H: Scramble reel Â· ??? EP
  H->>G: roll() free
  G->>E: client CSPRNG + evaluate
  E-->>G: number Â· badges Â· EP Â· rarity
  G->>S: history + collection + stats
  G-->>H: lastRoll source=client
  H->>H: Lock Â· cascade Â· count EP
  opt signed in
    G->>Sync: enqueueAutoSync
    Sync->>C: POST mode=delta pending rolls
    C-->>Sync: compact ack ok + counts
    Note over C: unlock notifs only â€” no Ranked crowns
  end
```

Auto-sync (v0.11+) sends a **delta** (`mode: 'delta'`, â‰¤60 pending rolls + new collection rows + absolute counters) and receives a compact `{ ok, updatedAt, counts }` ack â€” not a full `{ cloud }` round-trip. `GET /api/sync` remains the full-pull path; `pullFromCloud` merges that locally. Legacy full POST bodies are still accepted.
...
- Thin handler: [`api/webhooks/polar.ts`](../api/webhooks/polar.ts) â†’ [`server/polar/webhooks.ts`](../server/polar/webhooks.ts) + [`server/polar/entitlements.ts`](../server/polar/entitlements.ts).
...
- **Mandatory `src/lib/*-api.ts` wrappers** â€” UI must not call `fetch('/api/...')` directly (PNG `dataUrl` blob fetches are fine).
...
- **Zod** validates save import payloads, cloud sync POST bodies, public profile GET responses, best-roll / feature-request / arcade API payloads â€” not every endpoint.
...
- Some write handlers (`follow`, `me`, `attest`, sync orchestration) still keep more logic inline â€” prefer extracting when touching them.
...
- **Practice all-time / week (Total EP)** â€” public rolls with `source != ranked` (Free + Challenge). Shipped in **v0.16.7** (Practice no longer ranks combined `user_progress`).
- **All-Time (Total EP)** â€” `user_progress` lifetime EP / rolls / badge counts (synced overall progress: Free + Ranked + Challenge + journey EP). Same combined totals as the top-left HUD / journey. Shipped in **v0.16.7**.
- **Arcade** â€” `arcade_meta.best_run_score` (Digits); never mixes with EP boards.
- **Ranked all-time / week (Total EP)** â€” sum of public `source=ranked` rolls only.
- **Best Roll (`?view=best`)** â€” one personal best per player from public rolls matching scope/period; sort by EP or rarity (`RARITY_ORDER`); earliest `rolled_at` ties. Practice uses non-ranked rolls; **All-Time Best Roll** uses any public roll and (v0.17+) may expose `source` so the UI can show Free / Ranked / Challenge lane chips.
- **Period clocks (v0.16.5+)** â€” Ranked community â€œtodayâ€ / â€œthis weekâ€, leaderboard week, and crown windows use **UTC** calendar day / UTC ISO week; Ranked rolls/hour quota resets each UTC hour (`:00:00Z`).
- Client sync **cannot** set `source=ranked` (server preserves ranked on conflict).
- Sync **rejects** payloads that claim another userâ€™s roll ids or inflate EP/collection without matching rolls (`SyncIntegrityError` â†’ 409). Lifetime roll-count vs new-history is only enforced when client history is below `HISTORY_CAP` (500) â€” the counter is unbounded while history is retention-capped.
...
flowchart LR
  ...
  Merge --> Local[Codex Â· NEW filter Â· NEW ribbon]
...
flowchart TB
  ...
  Vanity -->|bot UA| HTML["/api/share Â· OG HTML"]
  HTML --> Img["/api/og PNG"]
  Profile["/u/:user"] -->|bot UA| UHTML["/api/u Â· profile OG"]
  UHTML --> Img
  Pages["/ Â· /leaderboard Â· /arcade Â· /features Â· /about Â· â€¦"] -->|bot UA| PageHTML["/api/page/:slug"]
...
| Claim                       | Reality                                                                                    |
| --------------------------- | ------------------------------------------------------------------------------------------ |
| Free-play randomness        | Browser CSPRNG + entropy pool â€” **client-authoritative**; Practice board honor system      |
...
| Attestation seal            | Server HMAC on a **claim** â€” not proof of honest client RNG                                |
...
| All-Time Best Roll lanes    | Optional `source` on best entries (`client` / `ranked` / `challenge`) â€” UI chips           |
| Ranked soft quota           | Free **90**/UTC hour; paid Rare/Epic/Anomaly via Polar entitlements; pill uses API `limit` |
| Leaderboard Arcade          | Best Digits run (`arcade_meta.best_run_score`); Digits â‰  EP; no crowns                     |
| Community crowns            | Ranked rolls only (UNION ALL tops; notifs only on wins)                                    |
| Share links                 | Only after roll row exists in Neon                                                         |
| Runtime schema validation   | Zod at **import / sync / profile / arcade** boundaries â€” not blanket on every API          |

## Related docs

- [README Status & roadmap](../README.md#-status--roadmap) â€” feature â†’ release tag map
- [Polar monetization](./polar-monetization.md) â€” products, webhooks, quota tiers
- [CHANGELOG](../CHANGELOG.md) â€” developer release notes
- [Arcade Mode design](./superpowers/specs/2026-07-09-arcade-mode-design.md)
- [Refactor notes (July 2026)](./refactor-notes-2026-07.md)
- [Opus audit + Â§H implementation status](./opus-report.md)
